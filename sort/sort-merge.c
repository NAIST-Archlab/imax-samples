...
//sort_quick(0, In, Out, 0, MAXSIZE-1);
  sort_merge(0, In, Out, 0, MAXSIZE-1);
...
sort_quick(int LANE, Data *In, Data *Out, int lo, int hi)
{
  int i = lo, j = hi, ref;
  Data tp;
  ...
  if (lo < j) sort_quick(0, In, Out, lo, j);    /* 上半分を整列 */
  if (i < hi) sort_quick(0, In, Out, i, hi);    /* 下半分を整列 */
  for (i=lo; i<=hi; i++)
    Out[i] = In[i];
}

sort_merge(int LANE, Data *In, Data *Out, int lo, int hi)
{
...
#if 0
  if ((n = ((hi-lo+1))/2) >= 2) {
    sort_merge(0, In, Out, lo, lo+(n-1)); /* 上半分を整列 */
    sort_merge(0, In, Out, hi-(n-1), hi); /* 下半分を整列 */
  }
  ...
  for (i=lo; i<=hi; i++)
    In[i] = Out[i];
#elif !defined(EMAX6) && !defined(EMAX7)
  printf("<<<ORIG>>>\n");
  reset_nanosec();
  BlockEnd = 1;
...
/*3*/for (CHIP=0; CHIP<NCHIP; CHIP++) {
 /*1*/for (INIT0=1,LOOP0=NumSamples,L8=0LL<<32|(0-8LL)&0xffffffff; LOOP0--; INIT0=0) { /* NumSamples<=4096 */
	...
	//if (Pipeline==3) printf("i=%d ja=%08.8x J=%d:%d.%d ka=%08.8x K=%d:%d.%d Out=%d\n", (Uint)i, (Uint)(J[3]+base), (Uint)J[3], (Uint)(BR[9][2][1]>>32), (Uint)BR[9][2][1], (Uint)(K[3]+base), (Uint)K[3], (Uint)(BR[9][2][0]>>32), (Uint)BR[9][2][0], Out[L8/8].val);
        ...
        sort_core1(23, 24,10, Buf[19],  0LL,                 8192LL,  Buf[20], Buf[21],0LL);         /* stage#23-25 */
        sort_core0(23, 24,11, 0xffffffffffff8000LL, Buf[21], 16384LL);                               /* stage#23-24 */
        sort_core1(25, 26,11, Buf[21],  0LL,                 16384LL, Buf[22], Buf[23],0LL);         /* stage#25-27 */
        sort_core0(25, 26,12, 0xffffffffffff0000LL, Buf[23], 32768LL);                               /* stage#25-26 */
        sort_core1(27, 28,12, Buf[23],  0LL,                 32768LL, Buf[24], Buf[25],0LL);         /* stage#27-29 */
        sort_core0(27, 28,13, 0xfffffffffffe0000LL, Buf[25], 65536LL);                               /* stage#27-28 */
        sort_core1(29, 30,13, Buf[25],  0LL,                 65536LL, Buf[26], Buf[27],0LL);         /* stage#29-31 */
        sort_core0(29, 30,14, 0xfffffffffffc0000LL, Buf[27],131072LL);                               /* stage#29-30 */
        sort_core1(31, 32,14, Buf[27],  0LL,                131072LL, Out,     Out,    NumSamples2); /* stage#31-33 */
#endif
      }
    }
//EMAX5A end
