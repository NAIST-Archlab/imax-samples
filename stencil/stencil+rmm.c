...
#if defined(EMAX7)
  reset_nanosec(0);
  grapes(0, D3D->D3D, GrA->GrA, B3D->B3D);
  get_nanosec(0,0);
  show_nanosec(0);
#else
  reset_nanosec();
...
#if defined(EMAX7)
  reset_nanosec(0);
  resid(0, D3D->D3D, B3D->B3D, C3D->C3D);
  get_nanosec(0,0);
  show_nanosec(0);
#else
  reset_nanosec();
...
#if defined(EMAX7)
    reset_nanosec(0);
    wave2d(0, WZ2->WZ2, WZ0->WZ0, WZ1->WZ1);
    get_nanosec(0,0);
    show_nanosec(0);
#else
    reset_nanosec();
...
grapes(int LANE, float *c, float *a, float *b )
     /*D3D[DP][HT][WD]*/
     /*GrA[XC][DP][HT][WD]*/
     /*B3D[DP][HT][WD]*/
{
...
#if !defined(EMAX5) && !defined(EMAX6) && !defined(EMAX7)
...
//EMAX5A begin grapes mapdist=1
   ...
   /*2*/for (INIT1=1,LOOP1=RMGRP,roofs=0-AWD*4; LOOP1--; INIT1=0) {      /* stage#0 *//* mapped to FOR() on BR[63][1][0] */
     /*1*/for (INIT0=1,LOOP0=AWD-PAD*2,coofs=(PAD-1)*4; LOOP0--; INIT0=0) {          /* stage#0 *//* mapped to FOR() on BR[63][0][0] */
	    ...
	    exe(OP_ADD3, &aofs,  atop[CHIP],  EXP_H3210, roofs, EXP_H3210,  coofs, EXP_H3210, OP_AND, 0x0000ffffffffffffLL, OP_NOP, 0LL); /* stage#1 */
	    exe(OP_ADD3, &bofs,  btop[CHIP],  EXP_H3210, roofs, EXP_H3210,  coofs, EXP_H3210, OP_AND, 0x0000ffffffffffffLL, OP_NOP, 0LL); /* stage#1 */
	    exe(OP_ADD3, &cofs,  ctop[CHIP],  EXP_H3210, roofs, EXP_H3210,  coofs, EXP_H3210, OP_AND, 0x0000ffffffffffffLL, OP_NOP, 0LL); /* stage#1 */
	    /*map0*/
...
jacobi(int LANE, float *c, float *b )
     /*D3D[DP][HT][WD]*/
     /*B3D[DP][HT][WD]*/
{
#undef  NCHIP
...
resid(int LANE, float *d, float *b, float *c )
     /*D3D[DP][HT][WD]*/
     /*B3D[DP][HT][WD]*/
     /*C3D[DP][HT][WD]*/
{
...
//EMAX5A begin resid mapdist=12 /* 12 */
   ...
   /*2*/for (INIT1=1,LOOP1=RMGRP,roofs=0-AWD*4; LOOP1--; INIT1=0) {      /* stage#0 *//* mapped to FOR() on BR[63][1][0] */
     /*1*/for (INIT0=1,LOOP0=AWD-PAD*2,coofs=(PAD-1)*4; LOOP0--; INIT0=0) {          /* stage#0 *//* mapped to FOR() on BR[63][0][0] */
	    ...
	    exe(OP_ADD3, &bofs,  btop[CHIP], EXP_H3210, roofs, EXP_H3210,  coofs, EXP_H3210, OP_AND, 0x0000ffffffffffffLL, OP_NOP, 0LL); /* stage#1 */
	    exe(OP_ADD3, &cofs,  ctop[CHIP], EXP_H3210, roofs, EXP_H3210,  coofs, EXP_H3210, OP_AND, 0x0000ffffffffffffLL, OP_NOP, 0LL); /* stage#1 */
	    exe(OP_ADD3, &dofs,  dtop[CHIP], EXP_H3210, roofs, EXP_H3210,  coofs, EXP_H3210, OP_AND, 0x0000ffffffffffffLL, OP_NOP, 0LL); /* stage#1 */
	    /*map0*/
...
wave2d(int LANE, float *z2, float *z0, float *z1 )
     /*WZ2[HT][WD]*/
     /*WZ0[HT][WD]*/
     /*WZ1[HT][WD]*/
{
...
//EMAX5A begin wave2d mapdist=8 /* 8 */
 ...
 /*2*/for (INIT1=1,LOOP1=RMGRP,roofs=0-AWD*4; LOOP1--; INIT1=0) {      /* stage#0 *//* mapped to FOR() on BR[63][1][0] */
   /*1*/for (INIT0=1,LOOP0=AWD-PAD*2,coofs=(PAD-1)*4; LOOP0--; INIT0=0) {          /* stage#0 *//* mapped to FOR() on BR[63][0][0] */
          exe(OP_ADD,  &coofs, INIT0?coofs:coofs, EXP_H3210, 4, EXP_H3210,  0LL, EXP_H3210, OP_AND, 0x00000000ffffffffLL, OP_NOP, 0LL); /* stage#0 */
	  exe(OP_ADD,  &roofs, roofs, EXP_H3210, INIT0?AWD*4:0, EXP_H3210,  0LL, EXP_H3210, OP_AND, 0x00000000ffffffffLL, OP_NOP, 0LL); /* stage#0 */
	  exe(OP_ADD3, &z0ofs, z0top[CHIP], EXP_H3210, roofs, EXP_H3210,  coofs, EXP_H3210, OP_AND, 0x0000ffffffffffffLL, OP_NOP, 0LL); /* stage#1 */
	  exe(OP_ADD3, &z1ofs, z1top[CHIP], EXP_H3210, roofs, EXP_H3210,  coofs, EXP_H3210, OP_AND, 0x0000ffffffffffffLL, OP_NOP, 0LL); /* stage#1 */
	  exe(OP_ADD3, &z2ofs, z2top[CHIP], EXP_H3210, roofs, EXP_H3210,  coofs, EXP_H3210, OP_AND, 0x0000ffffffffffffLL, OP_NOP, 0LL); /* stage#1 */
	  /*map0*/
